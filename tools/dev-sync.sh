...
# and reload the shell so QML changes take effect. End users do not need
# this — they get the plugin with `omarchy plugin add` (see README).
set -euo pipefail

ID="io.github.pixdevsapps.hertz-radio"
...
for f in manifest.json *.qml Model.js hertz-ctl genres.json README.md LICENSE; do
  [[ -e "$SRC/$f" ]] && cp "$SRC/$f" "$DEST/"
done
cp "$SRC"/shaders/station-tint.frag "$SRC"/shaders/station-tint.frag.qsb "$DEST/shaders/"
chmod 755 "$DEST/hertz-ctl"
